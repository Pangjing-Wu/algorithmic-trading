...
from strategies.vwap.m3t.micro.env import (BaselineTranche, HistoricalTranche,
                                           RecurrentTranche)
from strategies.vwap.m3t.micro.trader import MicroTrader
from strategies.vwap.m3t.model import (LSTM, HybridLSTM, Linear, MacroBaseline,
                                       MicroBaseline)
...
def main(args, config):
    ...
    micro_config = config['m3t']['micro']['model']
    ...
    for i in range(vp_data.n_tranche):
        train_vp = macro(vp_data.train_set[i].X).reshape(-1).numpy()
        test_vp  = macro(vp_data.test_set[i].X).reshape(-1).numpy()
        # micro dataset
    ...
    for i in range(tranches.n):
        if args.micro == 'Linear':
            micro = Linear(
                input_size=train_envs[0,0].observation_space_n, 
                output_size=train_envs[0,0].action_space_n,
                device=device
                )
        elif args.micro == 'HybridLSTM':
            micro = HybridLSTM(
        ...
        micro_file = os.path.join(config['model_dir'], 'm3t', 'micro',
    ...
    for envs in [train_envs, test_envs]:
        for t in range(envs.shape[0]):
            trade = dataset[envs[t,0]._data.date].trade
...
# python ./scripts/m3t/test.py --goal 200000 --stock 600000 --macro Baseline --micro Baseline --agent QLearning --reward dense --micro_episode 10000 --quote_length 5
