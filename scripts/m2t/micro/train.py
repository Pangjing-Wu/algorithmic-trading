...
from strategies.vwap.m2t.micro.datamgr import TrancheDataset
from strategies.vwap.m2t.micro.agent import QLearning
from strategies.vwap.m2t.micro.env import HistoricalTranche, RecurrentTranche
from strategies.vwap.m2t.model import HybridLSTM, Linear
...
def generate_tranche_envs(dataset, env, time, args, config):
    envs = list()
    for data in dataset:
        goal = random.sample(config['m2t']['micro']['goal_pool'], 1)[0]
...
def main(args, config):
    ...
    model_config = config['m2t']['micro']['model']
    if args.model == 'Linear':
        model = Linear(
    ...
    if args.agent == 'QLearning':
        agent = QLearning(
            ...
            optimizer=optimizer,
            device=device,
            **config['m2t']['micro']['agent']['QLearning']
            )
    else:
        raise ValueError('unkonwn agent.')

    model_dir = os.path.join(config['model_dir'], 'm2t', 'micro', args.stock, args.agent,
...
if __name__ == '__main__':
    # python -u ./scripts/m2t/micro/train.py --stock 600000 --episode 10000 --agent QLearning --eps 1.0 --i_tranche 1 --model Linear --reward dense --quote_length 20
    args  = parse_args()
