...
def transaction_matching(quote, trade, order)->tuple:
    '''
    arguments:
    ----------
    ...
    # arguments checking
    if type(quote) != pd.DataFrame:
        raise TypeError("argument type of quote must be pandas.DataFrame.")
    if type(trade) != pd.DataFrame:
        raise TypeError("argument type of trade must be pandas.DataFrame.")
    if type(order) != dict:
        raise TypeError("argument type of order must be dict.")
    if order['side'] not in ['buy', 'sell']:
        raise KeyError("argument value of order['side'] must be 'buy' or 'sell'.")
    if type(order['time']) != str:
        raise TypeError("argument type of order['time'] must be str.")
